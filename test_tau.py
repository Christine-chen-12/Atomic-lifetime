...
@pytest.mark.parametrize("df_up, df_up_low", test_data)
def test_process_df(df_up, df_up_low):
    # modefy the data
    ...
    def calc(x):
        ...
        Xif = x['average_partial_lifetime'] * R_i # Xif is the new partial lifetime 
        

        new_tau_i = 1/((1/Xif).sum())
        assert abs(T_i - new_tau_i) <=1e-8 # test the inequality 
...
def solve(atoms_file, in_dir1, in_dir2):
    test_data = []
    # function call
    ...
    for i in range(len(atoms_list)):
        ...
        test_data.append((df_up, df_up_low))
    return test_data


test_data += solve('neutral_atoms.xlsx', 'total_lifetime_neutral_data', 'partial_lifetime_neutral_data')
test_data += solve('singly_charged_ions.xlsx', 'total_lifetime_singly_charged_data', 'partial_lifetime_singly_charged_data')
